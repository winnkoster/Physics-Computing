...
y_vec = np.array([0.,32735.],float)         # initial [y_pos,y_vel], will be updated upon iteration
z_vec = np.array([0.,0.],float)         # initial [z_pos,z_vel], will be (eventually) updated upon iteration (when we add z variance)

orbital_energy_i = -u*(((x_vec[0]**2.)+(y_vec[0]**2.)+(z_vec[0]**2.))**(-0.5)) + 0.5*((x_vec[1]**2.)+(y_vec[1]**2.)+(z_vec[1]**2.))
...
orbital_energy_f = -u*(((x_pos_points[-1]**2.)+(y_pos_points[-1]**2.)+(z_pos_points[-1]**2.))**(-0.5)) + 0.5*((x_vel_points[-1]**2.)+(y_vel_points[-1]**2.)+(z_vel_points[-1]**2.))
...
plt.text(1.2e+11,0., 'Earth', fontsize=8)         # earth text
...
plt.plot([1.496e+11],[0.], 'o', color='green')      # earth plot
...
'''
plt.savefig('test_1b_orbit.png',dpi=300)
#plt.show()
