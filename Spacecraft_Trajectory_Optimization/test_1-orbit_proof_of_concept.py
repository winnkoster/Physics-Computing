...
x_vec = np.array([au_distance,10000.],float)         # initial [x_pos,x_vel], will be updated upon iteration
y_vec = np.array([0.,-2500.],float)         # initial [y_pos,y_vel], will be updated upon iteration
...
print(np.max(np.abs(x_vel_points)))
# 2D Trajectory Plot
plt.plot(x_pos_points,y_pos_points)
plt.xlabel('x [meters]')
plt.ylabel('y [meters]')
plt.tick_params(labelright=True)
plt.text(0,0, 'Sun',fontsize=8)   # sun point
plt.text(-2.279e+11,0., 'Mars', fontsize=8)
plt.plot([2], [1], 'o')
plt.plot([-2.279e+11],[0.], 'o', color='red')
