...
def trajectory_simulation_error(v_i):               # Takes initial velocity conditions array as ONE argument (v_i[0] = v_i_x, etc.), returns the distance from Mars using the given initial conditions. Will feed this to optimization routine to find where this function returns zero (or close to it)
    ...
    N = 1000                              # for fast test runs, don't use for real sims.
...
def optimization_algorithm(v_0_guess,earth_params_arg,mars_params_arg):
    ...
    while current_distance > stop_criterion:
        ...
        res = minimize(trajectory_simulation_error, v_0_guess, method='nelder-mead', options={'maxiter': 200, 'disp': True})
        print(res.x)
        print(' ')
        v_0_guess = res.x
        ...
        print(' ')
        if n_attempts > 5:      # sometimes it gets stuck. This will keep it from looping endlessly, and instead just return a super high value and keep going.
            print('Tried 5 times, this one will not converge! Breaking Loop...')
            velocity_vector = np.array( [99999.,99999.,99999.] )
...
# which entries of earth_data, mars_data to pull
#earth_list = [50,75,100]
#mars_list = [225,250,275]

skip = 20

velocity_array_tot = np.zeros( ( (len(earth_data)/skip)+1, 11 ) )
velocity_array_earth_only = np.zeros( ( (len(earth_data)/skip)+1, 11 ) )
velocity_array_mars_only = np.zeros( ( (len(earth_data)/skip)+1, 11 ) )
departure_day_info = np.zeros( ( (len(earth_data)/skip)+1, 11 ) )
arrival_day_info = np.zeros( ( (len(earth_data)/skip)+1, 11 ) )




#our new attempt to get a small, limited, but quick porckchop plot of at least some of the good stuff

i=0                # 47th entry of earth_data is August 17, 2024
ii = 0              # internal loop counter for append to array
while i < len(earth_data):      # 167th entry of earth_data is December 15, 2024
    j=107           # 107th entry of mars_data is March 15, 2025
    jj = 0          # internal loop counter
    while j < 320:  # 187th entry of mars_data is June 3, 2025
        print('iteration (i,j): '+str(i)+' '+str(j))
        guess = np.array( [earth_data[i,4]*1.5, earth_data[i,5]*1.5 , earth_data[i,6]] )
        v_spacecraft_i = optimization_algorithm(guess, earth_data[i], mars_data[j])
        v_spacecraft_f = trajectory_simulation_final_velocity(v_spacecraft_i)
        v_earth_i = np.array( [ earth_data[i,4],earth_data[i,5],earth_data[i,6] ] )
        v_mars_f = np.array( [ mars_data[j,4],mars_data[j,5],mars_data[j,6] ] )

        aa = orbital_params(v_spacecraft_i,v_spacecraft_f,v_earth_i,v_mars_f)
        velocity_array_earth_only[ ii, jj ] = aa[2]
        velocity_array_mars_only[ ii, jj ] = aa[5]
        velocity_array_tot[ ii , jj ] = aa[2] + aa[5]

        departure_day_info[ii,jj] = i
        arrival_day_info[ii,jj] = j
...
while i < len(earth_data):
    j=0
    while j < len(mars_data):
        print('iteration (i,j): '+str(i)+' '+str(j))
        guess = np.array( [earth_data[i,4]*1.5, earth_data[i,5]*1.5 , earth_data[i,6]*1.5] )
        v_spacecraft_i = optimization_algorithm(guess, earth_data[i], mars_data[j])
        v_spacecraft_f = trajectory_simulation_final_velocity(v_spacecraft_i)
        v_earth_i = np.array( [ earth_data[i,4],earth_data[i,5],earth_data[i,6] ] )
        v_mars_f = np.array( [ mars_data[j,4],mars_data[j,5],mars_data[j,6] ] )
        ...
        aa = orbital_params(v_spacecraft_i,v_spacecraft_f,v_earth_i,v_mars_f)
        velocity_array_earth_only[ i/skip, j/skip ] = aa[2]
        velocity_array_mars_only[ i/skip, j/skip ] = aa[5]
        velocity_array_tot[ i/skip , j/skip ] = aa[2] + aa[5]
...
print(' ')
print(velocity_array_earth_only)
print(velocity_array_mars_only)
print(velocity_array_tot)

np.savetxt('velocity_earth.txt',velocity_array_earth_only)
np.savetxt('velocity_mars.txt',velocity_array_mars_only)
np.savetxt('velocity_tot.txt',velocity_array_tot)
